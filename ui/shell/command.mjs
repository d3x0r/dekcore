...
5			else {
			...
			for( n = 1; n <= cmd.opts.min; n++ ) {
				...
				if( oc && oc.conflicts.length ) { // was already a conflict, update it.
					...
					if( n === newcmd.length )
					{
						debug_command_keys &&console.log( "Full command- guaranteed " + s );
						break;
					}
			...
			for( n = cmd.opts.min; n < newcmd.length; n++ )  {
				//console.log( "Setting:" +  newcmd.substr(0,n));
				Object.defineProperty( commands, newcmd.substr(0,n), {enumerable:debug_command_keys,configurable:true,value:cmd} );
			}
	...
	function buildPhrases( words ) {
		...
		export  function Transform(chunk, encoding, callback) {
			try {
				...
				//console.log( util.format( "read command transform caught error...", chunk, err ) );
		...
		Transform.prototype.setLastTell = (lastTold) => { this.lastTold = lastTold }


export { read_command as Filter };
