	...
	while (config.starts.length) {
		...
		if( p = startProc() )  { // return true to get the next ome... will get a resume later{
			if( p instanceof Promise ) {
				...
				p.then( config.resume ).catch( (err)=>{
...
async function loadConfig(path) {
	...
	return fc.init().then( ()=>{
			loadDefaults();
			return fc.reload( "config.run.jsox" ).then( loadRun )
		});
	...
	async function loadDefaults() {
		...
		if (!config.run.Λ)
			config.run.Λ = sack.id();
		else
			console.log("partial recovery??!");
		config.run.defaults = await import( "./config.jsox" );
	}
