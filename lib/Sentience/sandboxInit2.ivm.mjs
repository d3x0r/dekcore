...
// this is in a worker, but not in a vm.
...
/* Seal Sandbox */
if(0)
[/*"import",*/"eval", "Function", /*"module",*/ "console", "process", "require", "import", "sandbox"].forEach(key => {
    if( key in sandbox )
...
const importModules = new WeakMap();
...
global.setSync('emit', function(op) {
	coreThreadEventer.postMessage( op );
});

function invokeEvent( op, e ) {
	//console.trace( "Eval?", `on(${JSON.stringify( op)},${JSON.stringify(e)})` );
	context.eval( `on(${JSON.stringify( op)},${JSON.stringify(e)})` );
}

const importWaits = [];

function runScript_ext( filename, content, ismodule ) {
	const prom = runScript( filename, content, ismodule );
	console.log( "promp:", prom );
	if( prom ) prom.then( (module)=>{
		const i = importWaits.findIndex( (m)=>m===prom );
		if( i >= 0 ) importWaits.splice( i, 1 );
		console.log( "runScript_ext result:", module );
		return module;
	});
	importWaits.push( prom );

}
global.setSync('require', ivmRequire );
global.setSync('requireCode', runScript_ext );
...
function runScript( filename, content, isModule ) {
	...
	if( isModule ) {
		...
		return inst.then( (inst)=>{
			console.log( "Evaluate Sync: Import fails now?", filename );
...
function ivmRunFile( filename, isModule ) {
	...
	async function ivmModuleLinker(specifier, referencingModule) {
		...
		//const baseModule = referencingModule?imports.get( referencingModule.dependencySpecifiers );
		const oldModule = importModules.get( referencingModule );
		console.log( "oldModule?", referencingModule, oldModule );
		const altPath = importing.length>0?(importing[importing.length-1].path + "/" + specifier):specifier;
		if( referencingModule )
		console.log( "linker:Specifier:", specifier, altPath, referencingModule.dependencySpecifiers, importing );
		if( disk.exists( specifier ) || ( (usePath = altPath) && disk.exists( altPath ) ) ) {
			return ivmRunFile( usePath, true ).then( (module)=>{
				imports.set( specifier, module );
				return module
			});
		}
		console.log( "looking for:", specifier );
		const path = findModule( specifier );
		if( path ) {
			//sack.log( util.format( "loading:", path ) );
			return ivmRunFile( path, path.endsWith( ".mjs") ).then( (module)=>{
				imports.set( specifier, module );
				return module
			});
		}
...
globalThis.console = {log, trace(...a) { sack.log( util.format( new Error().stack ) + util.format( ...a ) ) } };
 globalThis.require = (specifier)=>{
	...
	const retval = module.exports;
	globalThis.module = {exports:{}};
	globalThis.exports = globalThis.module.exports; // clear for next time.
	return retval;
};
