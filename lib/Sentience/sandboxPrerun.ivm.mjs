const util = {
	format( ...a ) {
		...
		function toString(val,key) {
			level++;
			if( null === val ) { level--; return "null"; }
			if( undefined === val ) { level--; return "undefined"; }
			if( "string" === typeof val ) { level--; return val; }
			else if( "object" === typeof val ) {
				if( logged.get( val ) ) { level--; return "(cyclic)"; }
				logged.set( val, true );
				const result = '\n'+Object.keys(val).map( key=>`${Array(level+1).fill('\t').join('')}${key}:${Object.getOwnPropertyDescriptor( val, key ).get?"<getter>":toString(val[key],key)},\n` ).join('');
				logged.delete( val );
				level--;
				return  result;
			}
			if( "function" === typeof val ) { level--; return 'function ' + key + '()'; }
			level--;
			return String(val)
		}
	}
...
//doLog( "(2)global this?", globalThis );
...
// this is running in the VM.  What is the communication hook between VM and Worker Shell?

const coreThreadEventer = {
	postMessage(msg) {
		emit( msg );
		//console.log( "Send to worker parent - but this should be outside myself...", msg );
	}	,
	on( name, cb ) {
...
const emit_ = globalThis.emit;
{
	function emit(a,b) {
		emit_( JSOX.stringify( a ) );//{op:a, [a]:b} ) );
	}
...
function doLog(...args) {
	...
	log( util.format( new Error().stack ) );
	for( let b = 0; b * 1024 < s.length; b++ )
		log('sandboxPrerun:'+s.slice(b*1024,(b+1)*1024));
...
on( "message", (msg)=>{
	//doLog( "Got message:", typeof msg, msg );
...
function processMessage(msg, stream) {
	...
	if (msg.op === "run") {
		...
		//_debug_command_run && doLog("Run some code...", codeStack, msg);
		var res;
		try {
			const code = { file: msg.file, code: msg.code, id:msg.id, result: null }
			var codeIndex = codeStack.push(code)-1;
			//doLog( "RunInfo: " + msg.file );
			//console.log( "run code(delayed):"+ msg.code );			
			doLog( "Running code with import dynamic:" );
			const m = globalThis.requireCode( msg.file, msg.code, true );
			//.then( result=>{ doLog( "RequireCode resulted:", result);});
			//const f = new AsyncFunction( msg.code );
			//const res = f();
			const res = null;
			...
			if (res && (res instanceof Promise || Promise.resolve(res) === res || ("undefined" !== typeof res.then)))
				res.then(
					...
					//_debug_command_run &&
					 doLog( "Error caught from async code:", err.message, err.stack );
					if (err)
						reply(({
							op: "error"
			...
			doLog(util.format( "Run Failed:", err.message, err.stack, typeof err, err) );//, msg.code)
...
var fillSandbox = {
	...
	, async require(args, path) {
		//_debug_requires && 
		doLog("This is a thread that is doing a require in itself main loop", args, path, new Error().stack);
	...
	, io: {
		...
		getInterface(object, name) {
			var o = object;
			//doLog(util.format("getInterface called :", new Error().stack, this, object, name ))
	...
	, xon(event, callback) {
		this.console.log( "Is this somehow what on is?" );
...
//log( "Emit really should be a thing:", ("emit" in globalThis), ("has" in globalThis) );
emit( {op:'initDone'} );
