...
import text from "sack.vfs/text"
import commandStream from "../../command_stream_filter/command.js"
import labelStream from "../../command_stream_filter/label_insert.js"
...
export function Filter( e ) {
		...
		if( src ) {
			{
				...
				try {
					...
					vm.runInContext(code, sandbox 
						, { importModuleDynamically : vm.constants.USE_MAIN_CONTEXT_DEFAULT_LOADER, filename:src, lineOffset:0, columnOffset:0, displayErrors:true, timeout:500} )
				} catch( err ) {
		...
		return getObjects( sandbox, args, false, ( foundSandbox, foundName, where, nextArgs )=>{
			...
			if( where == "contains") {
				...
				return grab( foundSandbox )
						.catch( (err)=>{ output( "GRAB CAUGHT:"+err.toString()+"\n" )})
	...
	function Hold( sandbox, args ) {
		...
		return getObjects( sandbox, args, false, ( foundSandbox, foundName, where, nextArgs )=>{
			...
			if( where === "near" || where == "holding,holding" ) {
				...
				return hold( foundSandbox ).catch( (err)=>{ output( "HOLD ERROR:"+err.toString()+"\n" )});
			}else if( foundSandbox ) {
				console.log( "Error: object was found ", where, " instead of near or contained.")
			}
			return Promise.resolve( undefined );
	...
	function Inventory( sandbox, src ) {
		...
		const p = sandbox.nearObjects.then ( (i)=>{
			var contents = [];
			_debug_promise_resolution && sack.log( util.format(new Error().stack, "Inventory - near Objects resulted..." ));
	...
	function Exec( sandbox, src ) {
		...
		try {
			...
			output( "EXEC ERROR:"+err.toString() );
		throw err;
		}
		return null;
	}
