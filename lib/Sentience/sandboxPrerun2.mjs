...
function doLog(...args) {
	console.log('sandboxPrerun:',args);
	//console.log(s);
}
const resolvers = {};
const rejectors = {};
...
function processMessage(msg, stream) {
	...
	if (msg.op === "run") {
		...
		try {
			...
			var res = import( msg.file );
					//vmric(msg.code, sandbox.sandbox, { filename: msg.file.src, lineOffset: 0, columnOffset: 0, displayErrors: true });
	...
	const e = {
		...
		, post(name, ...args) {
			...
			return new Promise((resolve, reject) => {
				const thisId = mid++;
				postMessage({ op: 'e', o: Λ, id: thisId, e: name, args: args });
		...
		, postGetter(name) {
			...
			return new Promise((resolve, reject) => {
				const thisId = mid++;
				postMessage({ op: 'h', o: Λ, id: thisId, h: name });
				pendingOps.push({ id: thisId, cmd: name, resolve: resolve, reject: reject })
			})
		},
		...
		*/
		...
		idGen() {
			return sack.Id();//idGen.generator();
		},
		run(file, code) {
			if (!code) {
...
// these are all the code methods (and data overrides just in case)
// the Λ key probably shouldn't get duplicated back in?
var fillSandbox = {
	Λ: Λ
	, entity: entity
	, waiting: []
	...
	, post(name, ...args) {
		...
		return new Promise((resolve, reject) => {
			...
			postMessage({ op: 'f', id: thisId, f: name, args: args });
		});
		return p;
		/*block*/
	}
	...
	, async postGetter(name, ...args) {
		...
		var p = new Promise((resolv, reject) => {
			...
			const thisId = mid++;
			postMessage({ op: 'g', id: thisId, g: name });
...
console.log( "At end of prerun?", globalThis );