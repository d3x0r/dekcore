...
if( "undefined" !== typeof Λ ) {
		...
		filter_base = (await require( "./filter_base.js")).Filter;
...
class read_command extends filter_base {

		#state = { state : states.getCommand, command : null, args : [], words : null, slashes: 0 };
		#commandRegistry = [];
		#commands = {};
	#stree = {};
         inError = false;
	handleUnhandled  = null;
	constructor (entity, options_) {
	...
	forEach(...args) { 
		this.#commandRegistry.forEach(...args);
	}
//	const getCmds = commandRegistry.forEach.bind(commandRegistry);
//	Object.defineProperty( commands, "forEach", {enumerable:false, value:getCmds})
	//this._processCommand = _processCommand;

	//filter_base.Filter( this );
	//this.sandbox.GLOBAL=this.sandbox;
	RegisterCommand( name, opts, code ) {
		if( name === "unhandled" ) {
			this.handleUnhandled = code;
			return;
		}
		if( name in this.#commands ) {
				if (this.#commands[name].name === name ) throw new Error( "Command already registered" );
				else delete this.#commands[name];
		}
		...
		if( !this.#commandRegistry.find( (c,i)=>{
			if( c.name > newCode.name ){
				this.#commandRegistry.splice(  i,0, newCode );
				return true;
			}
			return false;
		}) )
			this.#commandRegistry.push(newCode);
		Object.defineProperty( this.#commands, name, {enumerable:debug_command_keys,configurable:true,value: newCode } );
		this.updateCommands( this.#stree, this.#commands, name );
	...
	updateCommands( stree, commands, newcmd ) {
		var cmd = this.#commands[newcmd];
		var id = newcmd;
		{
			var other;
			...
			if( !( other = stree[id[0]] ) ) {
				...
				for( n = cmd.opts.min; n < newcmd.length; n++ ) {
					Object.defineProperty( this.#commands, newcmd.substr(0,n), {enumerable:debug_command_keys,configurable:true,value:cmd} );
				}
			}
			else {
				...
				for( n = 1; n <= cmd.opts.min; n++ ) {
					...
					if( oc && oc.conflicts.length ) { // was already a conflict, update it.
						...
						// 'inv' 'inv2'
							...
							delete this.#commands[s];
							oc = this.#commands[s] = Object.assign({},oc);
							oc.conflicts = [oc,cmd];
							oc.code = function(args){
								//console.log( "Entity command output (command.js)THis should have worked.");
								return this.push( util.format( "Unclear command, please be more specific: ", this.conflicts.reduce((acc,val)=>acc += (acc.length?", ":"" )+ val.name, "" )+"." ) );
							}
						}else {
				...
				//this.#commands[newcmd.substr(0,n)] = cmd;
	...
	processCommand( chunk )
	{
		...
		if( this.#state.words && !this.inError  ) {
			if( this.handleUnhandled )
				this.handleUnhandled( this.#state.words.toString() );
			else
	...
	_processCommand ( word )
	{
		...
		case states.getCommandWord:
			if( word.text === '/' ) {
				...
				state.command = this.#commands[word.text];
				if( state.command ) {
					state.state = states.getCommandArgs;
				} else {
					this.inError = true;
					this.push( util.format( "Command not found", word.text ) );
		...
		case states.getCommandArgs:
			...
			if( word.text  ) {
				...
				if( err_ ) console.log( "ERROR IN COMMAND:"+err_.message );
		...
		default:
			...
			if( word.text ){
				...
				state.state = states.saveWords;
				saveword()
			} else {
				state.slashes = 0;
				state.state = states.getCommand; // text = null at end of line; reset parsing state.
			}
			break;
		}
		return
		...
		_transform (chunk, encoding, callback) {
			try {
				...
				this.push( "TRANFROM WITH ERROR:"+err.toString()+''+err.stack );
			}
			if( io.prompt )
				io.prompt( "\nEnter Command: ");
			else
		...
		_flush  (callback) { console.log( "stream flush?" ); callback(); }

		setLastTell(lastTold) { this.lastTold = lastTold }

}
function Filter( entity ) {
	return new read_command( entity );
}
